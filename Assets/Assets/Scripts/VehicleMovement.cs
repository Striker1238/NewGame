...
public class VehicleMovement : MonoBehaviour
{
	...
	void CalculatHover()
	{
        ...
        isOnGround = Physics.Raycast(ray, out hitInfo, maxGroundDist, whatIsGround);//Determine if the ship is on the ground by Raycasting down and seeing if it hits 

			lineRenderer.SetPosition(0, new Vector3(ray.origin.x, ray.origin.y, ray.origin.z)); //x,y and z position of the starting point of the line
			lineRenderer.SetPosition(1, new Vector3(transform.position.x, hitInfo.distance, transform.position.z)); //x,y and z position of the end point of the line
		...
		else
		{
			...
			var newFallGrafity = 0f;
			if (Input.GetKey(KeyCode.Space))
				newFallGrafity = -1 * fallGravity;
			else if (Input.GetKey(KeyCode.LeftShift))
                newFallGrafity = fallGravity;
			Vector3 gravity = -groundNormal * newFallGrafity;
            rigidBody.AddForce(gravity, ForceMode.Acceleration);
		}
